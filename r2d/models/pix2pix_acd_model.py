...
from saliency_sampler import Saliency_Sampler1
import numpy as np
import torch.nn.functional as F


class Pix2PixACDModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['G_GAN', 'G_L1','G_perceptual', 'D1_real', 'D1_fake','D2_real', 'D2_fake','D_cls','G_cls']
        # specify the images you want to save/display. The training/test scripts will call <BaseModel.get_current_visuals>
        self.visual_names = ['real_A', 'fake_B', 'real_B','want_B','tort_A','low_A']
        ...
        self.netG=Saliency_Sampler1(self.netG,saliency_network,task_input_size,saliency_input_size,self.device)

        if self.isTrain:  # define a discriminator; conditional GANs need to take both input and output images; Therefore, #channels for D is input_nc + output_nc
            self.netD1 = networks.define_D(6, opt.ndf, opt.netD,
                                          opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids,1)
            self.netD2 = networks.define_D(6, opt.ndf, opt.netD,
                                          opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids,0)
    ...
    def forward(self,p):
        ...
        self.fake_B,self.tort_A,self.low_A = self.netG(self.real_A,self.want_B,p)  # G(A)
    ...
    def backward_D1(self):
        ...
        pred_fake,pred_cls = self.netD1(fake_AB.detach())
        # print(pred_cls)
        # print(self.label)
        self.loss_D_cls = self.classification_loss(pred_cls,self.label)*20
        self.loss_D1_fake = self.criterionGAN(pred_fake, False,1)
        # Real
        real_AB = torch.cat((self.real_A, self.real_B), 1)
        # real_AB = torch.cat((real_AB, self.fake_B), 1)
        pred_real,pred_cls2 = self.netD1(real_AB.detach())
        # self.loss_D_cls2 = self.classification_loss(pred_cls2,self.label)
        self.loss_D1_real = self.criterionGAN(pred_real, True,1)
        # combine loss and calculate gradients
        self.loss_D1 = (self.loss_D1_fake + self.loss_D1_real) * 0.5 + self.loss_D_cls #+ self.loss_D_cls2
        self.loss_D1.backward()

    def backward_D2(self):
        """Calculate GAN loss for the discriminator"""
        # Fake; stop backprop to the generator by detaching fake_B
        fake_BB = torch.cat((self.want_B, self.fake_B), 1)  # we use conditional GANs; we need to feed both input and output to the discriminator
        # fake_AB = torch.cat((fake_AB, self.fake_B), 1)
        pred_fake = self.netD2(fake_BB.detach())
        # print(pred_cls)
        # print(self.label)
        # self.loss_D_cls = self.classification_loss(pred_cls,self.label)*20
        self.loss_D2_fake = self.criterionGAN(pred_fake, False,1)
        # Real
        real_BB = torch.cat((self.want_B, self.real_B), 1)
        # real_AB = torch.cat((real_AB, self.fake_B), 1)
        pred_real = self.netD2(real_BB.detach())
        # self.loss_D_cls2 = self.classification_loss(pred_cls2,self.label)
        self.loss_D2_real = self.criterionGAN(pred_real, True,1)
        # combine loss and calculate gradients
        self.loss_D2 = (self.loss_D2_fake + self.loss_D2_real) * 0.5 #+ self.loss_D_cls2
    ...
    def backward_G(self):
        ...
        fake_AB = torch.cat((self.real_A, self.fake_B), 1)  # we use conditional GANs; we need to feed both input and output to the discriminator
        fake_BB = torch.cat((self.want_B, self.fake_B), 1)
        # fake_AB = torch.cat((fake_AB, self.fake_B), 1)
        pred_fake1,pred_cls = self.netD1(fake_AB)
        pred_fake2 = self.netD2(fake_BB)
        self.loss_G_GAN = self.criterionGAN(pred_fake1, True,0)+self.criterionGAN(pred_fake2, True,0)
