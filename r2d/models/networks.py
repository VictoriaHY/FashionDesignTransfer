...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, gpu_ids=[],flag=0):
    ...
    if netD == 'basic':  # default PatchGAN classifier
        if flag==0:
            ...
            net = NLayerDiscriminator1(input_nc, ndf, n_layers_D, norm_layer=norm_layer)
        # else:
...
class GANLoss(nn.Module):
    ...
    def __call__(self, prediction, target_is_real,hinge):
        ...
        if self.gan_mode in ['lsgan', 'vanilla']:
            ...
            if hinge==0:
                ...
                if target_is_real:
                    loss = (F.relu(hinge-prediction)).mean()
                else:
                    loss = (F.relu(hinge+prediction)).mean()
        return loss
...
class TVLoss(nn.Module):
    ...
    def __call__(self, input):
        ...
        loss_tv = (x_tv / tv_x_size + y_tv / tv_y_size)
        return loss_tv
...
class NLayerDiscriminator1(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d):
        ...
        layers = []
    #     layers.append(nn.Conv2d(6, ndf, kernel_size=4, stride=2, padding=1))
    ...
    #     kernel_size = int(256 / np.power(2, 6))
    #     self.main = nn.Sequential(*layers)
    #     self.conv1 = nn.Conv2d(curr_dim, 1, kernel_size=3, stride=1, padding=1, bias=False)
    #     self.conv2 = nn.Conv2d(curr_dim, 4, kernel_size=kernel_size, bias=False)
    ...
    #     return out_src, out_cls.view(out_cls.size(0), out_cls.size(1))
        ...
        self.conv2=nn.Conv2d(ndf * nf_mult, 4, kernel_size=kw, stride=1, padding=padw)
    ...
    def forward(self, input):
        ...
        src=self.conv1(h)
        cl=self.conv2(h)
        return src,(cl.mean(2)).mean(2)#.view(cl.size(0), cl.size(1))
        # return self.model(input)
...
class PixelDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, norm_layer=nn.BatchNorm2d):
        ...
        self.conv1=nn.Conv2d(ndf * 2, 1, kernel_size=1, stride=1, padding=0, bias=use_bias)
        self.conv2=nn.Conv2d(ndf * 2, 4, kernel_size=1, stride=1, padding=0, bias=use_bias)
    ...
    def forward(self, input):
        ...
        src=self.conv1(h)
        cl=self.conv2(h)
        return src,cl.view(cl.size(0), cl.size(1))
