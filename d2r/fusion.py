...
if __name__ == '__main__':
    ...
    # hard-code some parameters for test
    opt1.dataroot="./data"
    opt1.name="hrawp+_pix2pix"
    opt1.model="pix2pix_new"
    opt1.direction="AtoB"
    print(opt1.model)
    model1 = create_model(opt1)      # create a model given opt.model and other options
    print(opt1.model)
    ...
    opt3 = TestOptions().parse()  # get test options
    # hard-code some parameters for test
    opt3.dataroot="./data"
    opt3.name="h_pix2pix"
    opt3.model="pix2pix"
    opt3.direction="AtoB"
    print(opt3.model)
    model3 = create_model(opt3)      # create a model given opt.model and other options
    print(opt3.model)
    model3.setup(opt3)
    for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):    # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_latest_freq>
        ...
        for i, data in enumerate(dataset):  # inner loop within one epoch
            ...
            model.set_input(data,model1,model2,model3)         # unpack data from dataset and apply preprocessing
            flag=0
            if epoch>250:
                flag=1
