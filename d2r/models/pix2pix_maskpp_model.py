...
class Pix2PixMaskppModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.visual_names = ['real_B', 'fake_B', 'fake_B_new','fake_B_new2','comb_B','mask1','mask2']
        ...
        self.netG = networks.define_G(6, 2, opt.ngf, opt.netG, opt.norm,
...
#            self.optimizers.append(self.optimizer_D)
    ...
    def set_input(self, input,model0,model1,model2):
        ...
        self.model0=model0
        self.model2=model2
        self.model0.set_input(input,model1)
        self.model2.set_input(input)
        self.set_requires_grad(self.model0.netG, False)
        self.set_requires_grad(self.model2.netG, False)
        with torch.no_grad():
            self.model0.forward()
            self.model2.forward(1)
            self.fake_B=self.model0.fake_B.detach()
            self.fake_B_new=self.model0.fake_B_new.detach()
            self.fake_B_new2=self.model2.fake_B.detach()
...
#        self.real_A = input['A' if AtoB else 'B'].to(self.device)
    ...
    def forward(self,p):
        ...
        self.comb_B = self.fake_B*(1+self.mask1)/2+self.fake_B_new*(1+self.mask2)/2+self.fake_B_new2*(1-(1+self.mask1)/2-(1+self.mask2)/2)
...
#        self.loss_G_GAN = self.criterionGAN(pred_fake, True,0)
        ...
        self.loss_TV = self.TV(self.mask1) + self.TV(self.mask2)
