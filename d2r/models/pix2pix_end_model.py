...
class Pix2PixEndModel(BaseModel):
    ...
    def __init__(self, opt,model0,model1):
        ...
        self.loss_names = ['G_perceptual',"TV"]
...
#        self.real_A = input['A' if AtoB else 'B'].to(self.device)
    ...
    def forward(self,p):
        ...
        self.fake_B,self.tort_A,_=self.model0.netG(self.real_A,p)
        self.fake_B_new,self.tort_A_new,_=self.model1.netG(self.real_A,p)
        self.mask = self.netG(torch.cat((self.real_B,self.fake_B_new),1))  # G(A)
        self.comb_B = self.real_B*(1+self.mask)/2+self.fake_B_new*(1-(1+self.mask)/2)
...
#        self.loss_G_GAN = self.criterionGAN(pred_fake, True,0)
        ...
        # combine loss and calculate gradients
        self.loss_G = self.loss_G_perceptual + self.loss_TV
